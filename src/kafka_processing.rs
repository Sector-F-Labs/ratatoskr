...
use teloxide::{
    payloads::{
        SendMessageSetters, SendPhotoSetters, SendAudioSetters, SendVoiceSetters,
        SendVideoSetters, SendVideoNoteSetters, SendStickerSetters, SendAnimationSetters,
        SendDocumentSetters, EditMessageTextSetters, EditMessageReplyMarkupSetters,
    },
    ...
    match message.message_type {
        ...
        OutgoingMessageType::EditMessage(data) => {
            ...
            if let Some(new_text) = data.new_text {
                let mut msg_to_edit = bot.edit_message_text(chat_id, teloxide::types::MessageId(data.message_id), new_text);
                ...
                if let Some(markup) = create_markup(&data.new_buttons) {
                    bot.edit_message_reply_markup(chat_id, teloxide::types::MessageId(data.message_id))
                        .reply_markup(markup)
                        .await?;
                }
            }
